...
class BritishCoins(object):
    def __init__(self, p1, p2, p5, p10, p20, p50, p100, p200):
        """assumes each input value is an integer
        representing how many of each coin is contained
        Represents a collection of British coins, of value 1p, 2p, 5p, 10p, 20p,
        50p,100p/1£, 200p/2£
        """
        self.p1 = p1
        self.p2 = p2
    ...
    def getTotalValue(self):
        ...
        return (self.p1*1) + (self.p2*2) + (self.p5*5) + (self.p10*10) + (self.p20*20)\
            + (self.p50*50) + (self.p100*100) + (self.p200*200)

    def __str__(self):
        """returns a list of how many of each coin is in the object"""
        rep_string = "1P coins:" + str(self.p1) + ", 2P coins:" + str(self.p2) + \
            ", 5P coins:" + str(self.p5) + ", 10P coins:" + str(self.p10) +\
            ", 20P coins:" + str(self.p20) + ", 50P coins:" + str(self.p50) +\
            ", 100P coins:" + str(self.p100) + ", 200P coins:" + str(self.p200)
        return rep_string
                        ...
                        for p50 in range(max_p50+1):
                            for p100 in range(max_p100+1):
                                for p200 in range(max_p200+1):
                                    a_BritishCoins = BritishCoins(p1, p2, p5, p10, p20,
                                                                  p50, p100, p200)
...
def circularPrimes(upper_limit):
    ...
    for prime in prime_list:
        ...
        for rotation in rotations_list:
            if isPrime(rotation) is False:
                all_rotations_prime = False
        if all_rotations_prime is True:
...
def circularPrimesAlt3(upper_limit):
    ...
    for num in range(upper_limit):
        ...
        all_rotations_prime = True
        for rotation in rotations_list:
            if isPrimeJosh(rotation) is False:
                all_rotations_prime = False
        if all_rotations_prime is True:
            circular_primes_list.append(num)
    return len(circular_primes_list)
...
# note: runtime of circularPrimes(1000000) ~15 hours
...
Find the sum of all numbers less than 1 million, which are palindromic in base10 and base2
...
def doublePalindromesSum(upper_limit):
    """assumes upper_limit is an int,
    representing the number up to which to look for double palindromes
...
# started at 5:28 pm, ended at 5:31 = runtime ~4 minutes
...
def largestConcatenatedPandigital():
    ...
    for num in range(9999):  # concatenation must be 9 digit
        products_list = []
        ...
        for product in products_list:
            ...
            if isPandigital(concatenated_products):
                ...
                if (largest_pandigital is None) or (new_pandigital > largest_pandigital):
...
def intRightAngleSolutions(p):
    ...
    each tuple representing (a,b,c), the lenghts of the legs a and b and hypothenus c
    of a right triangle of perimeter p
    the list of tuples representing all the possible integer solutions
    for a right triangle of perimeter p
    """
    perimeter_list = []
...
def bestIntRightTriangle(max_p):
    ...
    for p in range(1, max_p+1):
        ...
        if (p_with_max_solutions is None) or (lenght_p > lenght_p_with_max_solutions):
