...
def LargestPalindromeThreeDigitProduct():
    ...
    for num1 in range(100, 1000):
        for num2 in range(100, 1000):
            ...
            product_string = str(product)
            if product_string[:] == product_string[::-1]:
                if (largest_palindrome is False) or (product > largest_palindrome):
                    largest_palindrome = product
    return largest_palindrome
...
def smallestDivisible1to20():
    ...
    num = 20
    while found_divisible is False:
...
def sumOfSquaresMinusSquareofSum(ceiling):
    ...
    # calc sum of (squares) up to n
    sum_of_squares = 0
    for num in range(ceiling+1):
        squared_num = num**2
        sum_of_squares += squared_num
    # calc square of (sum of num up to n)
...
def buildGrid():
    ...
    str_list = [str1, str2, str3, str4, str5, str6, str7, str8, str9, str10, str11,
...
def convertToLetters(num):
    ...
    """
    single_digit_list = ["zero", "one", "two", "three", "four", "five", "six", "seven",
                         "eight", "nine"]
    noughts_list = ["", "one", "two", "three", "four", "five", "six", "seven", "eight",
                    "nine"]
    ...
    hundreds_list = noughts_list + tens_list + twenties_list + thirties_list +\
        forties_list + fifties_list + sixties_list + seventies_list + eighties_list\
        + nineties_list
...
def getTriangleValues(aTriangle, path):
    ...
    assumes aTriangle is a list of lists of ints,
    a representation of a root first binary tree
...
def getBestPathValue(aTriangle, start, path, best_value, best_path):
    ...
    possible_next_steps = (last_step_of_path[0]+1, last_step_of_path[1]),\
        (last_step_of_path[0]+1, last_step_of_path[1]+1)
    # recursively take each possible next step
    for step in possible_next_steps:
        if (best_path is None) or(calcTriangleSum(getTriangleValues(aTriangle, path)) < best_value):
        ...
        if (best_path is None) or (best_value is None) or (new_value > best_value):
...
class centuryDay (object):
    def __init__(self, year, month, day_of_month):
        ...
        assumes day_of_month is an int, representing the day (e.g. 1 for the 1st)
        assumes day_of_century is an int, representing the day of the century,
        starting at 1 (e.g. 1 is January 1st 1900, 368 is January 2nd 1901)
    ...
    def __str__(self):
        ...
        return str(self.day_of_month) + "." + str(self.month) + "." + str(self.year) +\
            ".." + str(self.day_of_century) + "." + str(self.weekday)
    ...
    def __repr__(self, verbose=True):
        ...
        """
        if verbose is True:
            return str(self.day_of_month) + "." + str(self.month) + "." +\
                str(self.year) + ".." + str(self.day_of_century) + "." + str(self.weekday)
        else:
            return str(self.day_of_month) + "." + str(self.month) + "." + str(self.year)
